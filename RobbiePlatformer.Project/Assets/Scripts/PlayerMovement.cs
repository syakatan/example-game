...
//このスクリプトは、ゲーム内のプレイヤーの動きと物理学を制御します

using UnityEngine;

public class PlayerMovement : MonoBehaviour
{
	public bool drawDebugRaycasts = true;   //Should the environment checks be visualized//環境チェックを視覚化する必要があります

	[Header("Movement Properties")]
	public float speed = 8f;                //Player speed//プレイヤーの速度
	public float crouchSpeedDivisor = 3f;   //Speed reduction when crouching//しゃがむときの速度低下
	public float coyoteDuration = .05f;		//How long the player can jump after falling//プレーヤーが落下してからジャンプできる時間
	public float maxFallSpeed = -25f;       //Max speed player can fall//最大速度のプレーヤーが落ちる可能性があります

	[Header("Jump Properties")]
	public float jumpForce = 6.3f;          //Initial force of jump//ジャンプの初期力
	public float crouchJumpBoost = 2.5f;    //Jump boost when crouching//しゃがんでいるときにジャンプブースト
	public float hangingJumpForce = 15f;    //Force of wall hanging jump//壁掛けジャンプの力
	public float jumpHoldForce = 1.9f;      //Incremental force when jump is held//ジャンプが保持されているときの増分力
	public float jumpHoldDuration = .1f;    //How long the jump key can be held//ジャンプキーを保持できる時間

	[Header("Environment Check Properties")]
	public float footOffset = .4f;          //X Offset of feet raycast// Xフィートのレイキャスト
	public float eyeHeight = 1.5f;          //Height of wall checks//壁の高さチェック
	public float reachOffset = .7f;         //X offset for wall grabbing//壁をつかむためのXオフセット
	public float headClearance = .5f;       //Space needed above the player's head//プレイヤーの頭の上に必要なスペース
	public float groundDistance = .2f;      //Distance player is considered to be on the ground//距離プレーヤーは地面にいると見なされます
	public float grabDistance = .4f;        //The reach distance for wall grabs//壁グラブの到達距離
	public LayerMask groundLayer;           //Layer of the ground//地面の層

	[Header ("Status Flags")]
	public bool isOnGround;                 //Is the player on the ground?//プレイヤーは地面にいますか？
	public bool isJumping;                  //Is player jumping?//プレイヤーはジャンプしていますか？
	public bool isHanging;                  //Is player hanging?//プレイヤーはハングしていますか？
	public bool isCrouching;                //Is player crouching?//プレーヤーはしゃがんでいますか？
	public bool isHeadBlocked;

	PlayerInput input;                      //The current inputs for the player//プレーヤーの現在の入力
	BoxCollider2D bodyCollider;             //The collider component//コライダーコンポーネント
	Rigidbody2D rigidBody;                  //The rigidbody component// Rigidbodyコンポーネント

	float jumpTime;                         //Variable to hold jump duration//ジャンプ時間を保持する変数
	float coyoteTime;                       //Variable to hold coyote duration//コヨーテの持続時間を保持する変数
	float playerHeight;                     //Height of the player//プレーヤーの高さ
